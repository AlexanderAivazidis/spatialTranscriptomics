...
for (i in 1:length(focusGroupsList)){
  ...
  for (j in 1:length(clustersToMergeList)){
   ...
   for (k in 1:length(lossFunctionList)){
     results = getFishPanelAndFScore(data, runGenes, specific_type, medianExpr, newWeightMatrix, panelSize = 96, focusGroup = focusGroupsList[[i]][focusGroupsList[[i]] %in% specific_type], 
