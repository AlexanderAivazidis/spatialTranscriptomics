# Poster Figure 2 and 3: This figure shows performance on mapping 56 glutamatergic neuron types as a function of genes included (1 to 36, ALM or VISp).
...
minOn = 0.1
focusGroup = unique(specific_type[coldata['class'] == 'Glutamatergic'])
...
for (k in c(1,2)){
  ...
  for (pS in 1:panelSize){
    ...
    save(fishPanel, file = paste(savingDirectory, 'markerGenes/allen_markerGenesGlutamatergicNeurons_ALM_36genes_lossFunction', as.character(k), '.RData', sep = ""))
  }
}
